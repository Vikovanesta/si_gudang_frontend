...
const fetchBorrowedItems = async () => {
  try {
    const response = await axios.get('/borrowed-items', {
      params: {
        q: searchQuery.value,
    ...
    <VCard id="registration-list">
      ...
      >
        ...
        <template #item.borrower="{ item }">
          <div :id="`borrower-${item.id}`">
            {{ item.borrower.profile?.name }}
          </div>
        </template>
        ...
        <template #item.actions="{ item }">
          <div class="text-no-wrap">
            ...
            <IconBtn
              :id="`btn-view-${item.id}`"
              size="small"
              :to="{ name: 'app-riwayat-peminjaman-detail-id', params: { id: item.id } }"
            >
